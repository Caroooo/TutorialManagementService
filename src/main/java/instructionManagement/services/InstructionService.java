package instructionManagement.services;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import instructionManagement.api.mapper.InstructionChildStepMapper;
import instructionManagement.api.mapper.InstructionStepMapper;
import instructionManagement.api.mapper.InstructionMapper;
import instructionManagement.api.model.InstructionChildStepCreateView;
import instructionManagement.api.model.InstructionCreateView;
import instructionManagement.api.model.InstructionStepCreateView;
import instructionManagement.model.Instruction;
import instructionManagement.model.InstructionChildStep;
import instructionManagement.model.InstructionStep;
import instructionManagement.model.Resource;
import instructionManagement.repositories.InstructionChildStepRepository;
import instructionManagement.repositories.InstructionRepository;
import instructionManagement.repositories.InstructionStepRepository;

import java.util.LinkedList;
import java.util.List;
import java.util.Optional;

@Service
public class InstructionService {

    @Autowired
    private InstructionRepository instructionRepository;
    @Autowired
    private InstructionStepRepository instructionStepRepository;
    @Autowired
    private InstructionChildStepRepository instructionChildStepRepository;
    @Autowired
    private ResourceService resourceService;
    @Autowired
    private InstructionMapper instructionMapper;
    @Autowired
    private InstructionChildStepMapper instructionChildStepMapper;
    @Autowired
    private InstructionStepMapper instructionStepMapper;

    public void addInstruction(InstructionCreateView instructionCreateView){
        List<InstructionStep> steps = new LinkedList<>();
        for(InstructionStepCreateView instructionStepCreateView : instructionCreateView.getSteps()){
            InstructionStep instructionStep = instructionStepMapper.createViewToInstructionStep(instructionStepCreateView);
            List<InstructionChildStep> childSteps = new LinkedList<>();
            for(InstructionChildStepCreateView childStepCreateView : instructionStepCreateView.getInstructionChildSteps()){
                InstructionChildStep childStep = instructionChildStepMapper.createViewToInstructionChildStep(childStepCreateView);
                Optional<instructionManagement.model.Resource> resource = resourceService.getResourceById(childStepCreateView.getResourceId());
                if(resource.isPresent()){
                    childStep.setResource(resource.get());
                }
                childSteps.add(childStep);
            }
            instructionStep.setInstructionChildSteps(childSteps);
            steps.add(instructionStep);
        }
        Instruction instruction = new Instruction();
        instruction.setTitel(instructionCreateView.getTitel());
        instruction.setInstructionType(instructionCreateView.getInstructionType());
        instruction.setShortDescription(instructionCreateView.getShortDescription());
        instruction.setSteps(steps);
        instructionRepository.save(instruction);
    }

    public List<Instruction> getInstructions(){

       // tutorialChildStep.setImage("data:image/png;base64,");

      return instructionRepository.findAll();
    }

    public Optional<Instruction> getInstructionById(long id) {

        Optional<Instruction> instruction = instructionRepository.findById(id);

        return  instruction;
       //String audio = getAudioAsBase64("C:\\Users\\carom\\Documents\\FHV\\Master\\Angular-Projects\\resourcen\\sound.mp3");

    }


    public void setResourceToChildStep(long childStepId, long resourceId) {
        Optional<InstructionChildStep> childStep = instructionChildStepRepository.findById(childStepId);
        Optional<Resource> resource = resourceService.getResourceById(resourceId);

        if(childStep.isPresent() && resource.isPresent()){
            childStep.get().setResource(resource.get());
            resource.get().setInstructionChildStep(childStep.get());
            instructionChildStepRepository.save(childStep.get());
            resourceService.saveResource(resource.get());
        }
    }
}
